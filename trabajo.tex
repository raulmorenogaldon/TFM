...
Tomando un ejemplo donde hay un rango de calidades $[0,49]$, 100 ciclos de secuenciador y 12 tipos de dinucleótido, el cálculo de los deltas se traduce en un delta global, 50 deltas (uno para cada posible calidad), $50*100=5000$ deltas para cada par calidad-ciclo y $50*12=600$ deltas para cada par calidad-dinucleótido. Esto se traduce en el cálculo de $5651$ deltas.\\
...
Las matrices para el delta global tiene dimensión $1x1$, puesto que sólo hay un caso. Las matrices para el delta de posición tiene dimensión $1xn$ siendo $n$ el número de posibles calidades que puede tener una posición. Para los delta de posición-ciclo y posición-dinucleótido la dimensión es $nxc$ y $nxd$ respectivamente, siendo $c$ el número de ciclos y $d$ el número de dinucleótidos (lo normal es que $d$ sea 12).\\

Por ejemplo, tenemos un fichero \textit{BAM} de un secuenciador de 100 ciclos, con 35 millones de lecturas ($35\exp^6$) tendrá aproximadamente 3500 millones de bases ($3,5\exp^9$), con calidades comprendidas en el rango $[0,50]$. Las estructuras de datos utilizadas en este caso serán:
\begin{itemize}
\item
Para el delta global: tres matrices de dimensión $1x1$ que contengan el número de bases leídas en total, el número de fallos y su delta. Hacen un total de 2 enteros y un número en coma flotante (\textit{double} en C).
\item
Para el delta de cada calidad: tres matrices de dimensión $1x50$ que contengan el número de bases leídas que tengan esa calidad, sus fallos y su delta. Hacen un total de 100 enteros y 50 \textit{double}.
\item
Para el delta de cada calidad-ciclo: tres matrices de dimensión $50x100$ que contengan el número de bases leídas que tengan esa calidad en ese ciclo, sus fallos y su delta. Hacen un total de 10000 enteros y 5000 \textit{double}.
\item
Para el delta de cada calidad-dinucleótido: tres matrices de dimensión $50x12$ que contengan el número de bases leídas que tengan esa dinucleótido, sus fallos y su delta. Hacen un total de 1200 enteros y 600 \textit{double}.
\end{itemize}
