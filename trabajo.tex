...
Q = -10 \times \log_{10} P
\end{equation} 
cuya entrada es un rango de números reales [0,1] y su salida en el rango de números naturales positivos [0,93] (acotado superiormente). La razón de utilizar esta calidad es que solo es necesario usar un byte para su expresión, aunque se pierda precisión en la probabilidad de error.\\
...
Esta tarea se ha iniciado de forma parcial. Ya se ha realizado alguna prueba sobre las instrucciones SSE que se van incorporando. cuyos resultados se encuentran. En la sección \ref{sec:evaluacion} se verá el rendimiento obtenido al utilizar estas instrucciones.\\
...
El otro programa que he utilizado para depuración es \textit{Valgrind} \cite{valgrindweb}. Es un conjunto de herramientas de depuración que permiten detectar automáticamente muchos fallos en el manejo de memoria e hilos, permitiendo analizar los programas en detalle.\\

Durante el desarrollo del recalibrador he utilizado la herramienta \textit{memcheck} de \textit{Valgrind}. Esta herramienta permite detectar errores en la memoria, como accesos no válidos, uso de variables no inicializadas, liberación incorrecta de memoria reservada, fugas de memoria\ldots El principal uso que le he dado ha sido localizar puntos del código del recalibrador donde no se liberaba bien la memoria, provocando que la memoria libre se agote y disminuyendo el rendimiento de forma drástica.\\
...
En la segunda fase se vuelve a recorrer el fichero para aplicar la fórmula \ref{for:qualrecal} que devuelve la calidad recalibrada para cada base leída. Puesto que tenemos todos los deltas calculados en las estructuras de datos, sólo nos llevará 4 sumas el cálculo de cada calidad. \\
...
\begin{itemize}
\item
Para el delta global: tres matrices de dimensión $1 \times 1$ que contengan el número de bases leídas en total, el número de fallos y su delta. Hacen un total de 2 enteros y un número en coma flotante (\textit{double} en C).
\item
Para el delta de cada calidad: tres matrices de dimensión $1 \times 50$ que contengan el número de bases leídas que tengan esa calidad, sus fallos y su delta. Hacen un total de 100 enteros y 50 \textit{double}.
\item
Para el delta de cada calidad-ciclo: tres matrices de dimensión $50 \times 100$ que contengan el número de bases leídas que tengan esa calidad en ese ciclo, sus fallos y su delta. Hacen un total de 10000 enteros y 5000 \textit{double}.
\item
Para el delta de cada calidad-dinucleótido: tres matrices de dimensión $50 \times 12$ que contengan el número de bases leídas que tengan esa dinucleótido, sus fallos y su delta. Hacen un total de 1200 enteros y 600 \textit{double}.
\end{itemize}

En el ejemplo anterior tenemos finalmente unos requisitos de memoria de 11302 enteros y 5651 \textit{double}. Teniendo en cuenta que el tamaño de un entero largo son 4 bytes y los tipo \textit{double} son 8 bytes, necesitaremos $90416$ bytes (90,5Kb aproximadamente). Actualmente 90 Kb es un tamaño muy pequeño para una memoria de un sistema de altas prestaciones, por lo que el sobrecoste de precalcular los deltas es despreciable. Con esto se ha conseguido un uso eficiente de la memoria.\\
...
La segunda fase se ejecuta con el parámetro \verb+-F+ o \verb+-2+. Esta fase tiene como parámetros obligatorios un fichero BAM de entrada obligatorio (\verb+-I+), el número máximo de ciclos (\verb+-C+), y un fichero de salida para el BAM recalibrado (\verb+-o+). Si se ejecuta esta fase sin haber ejecutado anteriormente la primera (sólo con el parámetro \verb+-2+), es necesario especificar el fichero de datos que se utilizará y que se generó en la primera fase (\verb+-d+).\\
...
Las primeras pruebas se realizaron una vez implementada la versión secuencial del nuevo recalibrador, usando las estructuras de datos optimizadas pero sin utilizar las optimizaciones automáticas que el compilador ofrece. El objetivo era obtener el rendimiento que se obtiene con la nueva herramienta comparándolo con el obtenido en las mismas condiciones para la herramienta recalibrador que incorpora GATK.\\

El entorno de prueba se detalla en la Tabla \ref{tab:specprueba}. Consistía en un sistema de un solo procesador de 4 núcleos de procesamiento con 4 GB de memoria principal. Las pruebas se han realizado utilizando un BAM de entrada con $35 \times 10^6$ lecturas, que con 100 ciclos por entrada hacen un total de $3.5 \times 10^9$ bases a recalibrar. Además sólo se ha utilizado un núcleo del procesador.\\
...
Número total de bases			& $35 \times 10^6 \times 100 \textbf{ciclos} = 3.5 \times 10^9$ bases\\\hline
...
Los dos recalibradores han realizado el mismo proceso de recalibrado en las mismas condiciones, teniendo en cuenta la posición de las bases, su ciclo de secuenciador y su entorno de dinucleótido. Las pruebas se pueden dividir en tres, primeramente la comparación de la herramienta de GATK con el recalibrador desarrollado sin SSE, siendo la segunda prueba una comparación entre la versión del recalibrador sin SSE con la que lo implementa en una pequeña proporción. Finalmente la ultima prueba compara el rendimiento obtenido por el recalibrador con las opciones de optimización (\verb+-O3+ de GCC) activadas.\\

\subsection[Evaluación del recalibrador sin optimizar]{Evaluación del recalibrador sin SSE ni optimizaciones de compilador}
...
Al evaluar el tiempo de ejecución de la versión del recalibrador que utiliza únicamente las estructuras de datos optimizadas, se obtuvo el BAM resultado en 880 segundos. Al igual que con GATK se obtiene el \textit{throughput} y se obtiene $\frac{3.5 \times 10^9}{880} \approx 3,98 \times 10^6$ bases/segundo.\\

Comparando los dos \textit{throughput}, el nuevo recalibrador es mucho más productivo que el implementado por GATK, en términos de \textit{SpeedUp} se ha conseguido $\frac{2685}{880} = \textbf{3,05}$ como factor de aceleración. Este factor nos indica que efectivamente la herramienta desarrollada es más rápida que la herramienta de GATK.\\

\subsection[Evaluación del recalibrador con SSE sin optimizar]{Evaluación del recalibrador con SSE sin optimizaciones del compilador}

Esta prueba pretende comparar el rendimiento del recalibrador cuando implementa instrucciones SSE2, respecto a la versión anterior sin SSE. En este caso tampoco estaban activadas las opciones de optimización del compilador. Puesto que sólo las implementa actualmente para la primera fase de recalibrado, solo se tendrán en cuenta los tiempos en esta fase, concretamente en lo que tarda en procesar una lectura (de media). Para procesar una lectura sobre el fichero BAM de prueba se necesitaron de media 7.36 microsegundos usando SSE. En la versión sin SSE el procesador de cada lectura tardó de media 7.57 microsegundos, lo que indica que hubo una mejora de rendimiento menor, es decir, con SSE es un 2\% - 3\% más rápido. \\

Aún siendo una pequeña mejora de rendimiento, sólo se ha utilizado SSE en una pequeña parte del código total, por lo que esperamos sacar más rendimiento cuando lo implementemos en el resto.\\

\subsection[Evaluación del recalibrador con optimizaciones]{Evaluación del recalibrador con optimizaciones del compilador}

Para estas pruebas se activaron las optimizaciones que el compilador GCC implementa automáticamente mediante el parámetro \verb+-O3+. Hay que tener en cuenta que entre estas mejoras se encuentra la utilización automática de instrucciones SSE en los sistemas de 64 bits. Con estas optimizaciones el recalibrador procesó el fichero BAM en sólo 645 segundos, por lo que si lo comparamos directamente con GATK obtenemos un \textit{SpeedUp} de $\frac{2685}{645} = \textbf{4,16}$. Lo cual nos indica que el nuevo recalibrador es hasta 4 veces más rápido que el de GATK en un solo núcleo de procesamiento.\\

\section{Conclusiones}

Actualmente se está implementando SSE de forma manual en algunas partes de código que son críticas en el rendimiento, con la intención de aprovechar al máximo los recursos de computo de los que se dispone. Esto, junto a la implementación automática de SSE y otras técnicas de optimización del compilador, permite aumentar la eficiencia como ya se ha visto en este capítulo. El siguiente paso puede ser utilizar SSE3 o SSE4, que no son implementadas automáticamente y pueden afectar al rendimiento. Otra razón para implementar manualmente SSE, es que en procesadores de 32 bits, el compilador no genera código SSE automáticamente, por lo que se perdería esta ventaja. También se perdería si utilizamos otro compilador que tampoco genere este tipo de código de forma automática.\\
