...
Al evaluar el tiempo de ejecución de la versión del recalibrador que utiliza únicamente las estructuras de datos optimizadas, se obtuvo el BAM resultado en 830 segundos. Al igual que con GATK se obtiene el \textit{throughput} y se obtiene $\frac{3.5 \times 10^9}{830} \approx 4,22 \times 10^6$ bases/segundo.\\

Comparando los dos \textit{throughput}, el nuevo recalibrador es mucho más productivo que el implementado por GATK, en términos de \textit{SpeedUp} se ha conseguido $\frac{2685}{830} = \textbf{3,23}$ como factor de aceleración. Este factor nos indica que efectivamente la herramienta desarrollada es mucho más rápida que la herramienta de GATK.\\

\subsection{Evaluación del recalibrador con SSE}

Esta prueba pretende comparar el rendimiento del recalibrador cuando implementa instrucciones SSE2, respecto a la versión anterior sin SSE. Puesto que sólo las implementa actualmente para la primera fase de recalibrado, solo se tendrán en cuenta los tiempos en esta fase. Para realizar la primera fase sobre el fichero BAM de prueba se necesitaron 179 segundos usando SSE. En la versión sin SSE esta fase tardó 183 segundos, lo que indica que hubo una mejora de rendimiento menor, es decir, con SSE es un 2\% más rápido. \\

Aún con esta pequeña mejora de rendimiento, sólo se ha utilizado SSE en una pequeña parte del código, por lo que podremos sacar más rendimiento cuando lo implementemos en el resto.\\
