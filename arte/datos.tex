\chapter{Estado del Arte: Tratamiento de grandes cantidades de datos}
...
\item
\textbf{Variedad}: No solo se manejan muchos datos, sino además estos pueden tener distinta estructura como pueden ser texto, audio, vídeo, etc.
\end{itemize}
...
Actualmente hay mucho software que implementa este modelo como puede ser el conocido \textit{Hadoop} \cite{bigdataglos, hadoopweb}, \textit{Hive} \cite{hiveweb}, \textit{Cascalog} \cite{cascalogweb}, \textit{mrjob} \cite{mrjobweb} \ldots A continuación se comentará \textit{Hadoop} entre otros.\\

\section{Hadoop}

\textit{Hadoop} es un proyecto open-source diseñado para el procesamiento de grandes cantidades datos. Es software fiable y escalable que se basa en computación distribuida. \textit{Hadoop} se encarga de ejecutar las aplicaciones en un entorno distribuido como puede ser un cluster.\\
...
En esta \textit{tesis} se tendrá en cuenta, como base, el modelo \textit{MapReduce}. Lo que se pretende es utilizar un modelo similar, parecido al \textit{scatter-gather}, que nos permita leer por partes o lotes el dataset de entrada. Estos lotes serán enviados a procesar a distintos procesadores por orden de lectura y obteniendo los resultados de los mismos de forma parcial. Cuando un lote es procesado se lee el siguiente y se procesa. La cantidad de lotes leídos simultáneamente podría basarse en el factor de memoria disponible en todos los nodos.\\